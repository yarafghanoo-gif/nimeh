...
async function handleLike() {
  ...
  if (isMatch) {
    ...
    await setDoc(doc(db, "chats", chatId), { participants: [currentUser.uid, target.id], createdAt: new Date(), lastMessage: "" });
...
function openChatRoom(chatId, otherUserId) {
  ...
  getDoc(doc(db, "users", otherUserId)).then(userDoc => {
    if (userDoc.exists()) document.getElementById("chat-user-name").innerText = userDoc.data().displayName;
  });
...
async function saveProfile() {
  ...
  const updateData = { displayName: name || "کاربر", age: isNaN(age) ? null : age, bio, interests };
...
document.getElementById("email-login").onclick = async () => {
  ...
  try { await signInWithEmail(email, password); } catch (err) { alert(err.message); }
};
...
document.getElementById("message-input").addEventListener("keypress", (e) => { if (e.key === "Enter") sendMessageFromInput(); });
